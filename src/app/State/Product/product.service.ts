		...
		return this.http.get(`${this.API_BASE_URL}/api/products`, { headers, params }).pipe(
			...
			catchError((error: any) => {
				...
				return of(
					...
					findProductByIdFailure(error.response && error.response.data.message ? error.response.data.message : error.message)
				);
			})
		...
		return this.http.get(`${this.API_BASE_URL}/api/products/id/${productId}`, { headers }).pipe(
			...
			catchError((error: any) => {
				...
				return of(
					...
					findProductByCategoryFailure(error.response && error.response.data.message ? error.response.data.message : error.message)
				);
			})
		).subscribe((action) => this.store.dispatch(action))
	}
