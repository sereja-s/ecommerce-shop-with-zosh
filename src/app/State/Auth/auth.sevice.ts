		...
		this.http.post(`${this.apiUrl}/signin`, loginData).pipe(
			...
			catchError((error) => {
				...
				return of(
					...
					loginFailure(error.response && error.response.data.message ? error.response.data.message : error.message)
				)
			})
		...
		this.http.post(`${this.apiUrl}/signup`, user).pipe(
			...
			catchError((error) => {
				...
				return of(
					...
					registerFailure(error.response && error.response.data.message ? error.response.data.message : error.message)
				)
			})
		).subscribe((action) => this.store.dispatch(action))
	}
